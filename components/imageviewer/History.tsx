...
const History = () => {
  ...
  const [showStorageError, setShowStorageError] = useState(false); // State for showing storage error
    ...
    if (isClient && pastImages.length > 0) {
      try {
        ...
        if (e instanceof DOMException && e.name === "QuotaExceededError") {
          ...
          setShowStorageError(true); // Show the error message
...
<div className="relative">
      ...
      <ErrorMessage
        message="Storage space limit reached"
        show={showStorageError}
      />
    </div>
